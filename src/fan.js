...
export function fanSVG(c,time,transparent=false){const s=fanScene(c,time);return `<svg xmlns="http://www.w3.org/2000/svg" width="${s.width}" height="${s.height}" viewBox="0 0 ${s.width} ${s.height}">${transparent?'':`<rect width="100%" height="100%" fill="${s.background}"/>`}${s.leaves.map((l,i)=>`<path id="folha-${i+1}" fill="${l.color}" transform="matrix(${l.matrix.join(' ')})" d="${SYMBOL}"/>`).join('')}</svg>`;}
export function createFanRenderer(c,{scale=1,transparent=false}={}){
 ...
 function renderAt(time,settings=c){const s=fanScene(settings,time);ctx.setTransform(scale,0,0,scale,0,0);ctx.clearRect(0,0,s.width,s.height);if(!transparent){ctx.fillStyle=s.background;ctx.fillRect(0,0,s.width,s.height);}for(const l of s.leaves){ctx.save();ctx.transform(...l.matrix);ctx.fillStyle=l.color;ctx.fill(path);ctx.restore();}}
